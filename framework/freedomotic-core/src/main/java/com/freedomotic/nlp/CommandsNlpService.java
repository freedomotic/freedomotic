 ...
 * @author enrico
 */
public class CommandsNlpService implements BusConsumer {
    ...
    private static final String PROPERTY_NLP_TEXT = "text";
    ...
    public void onMessage(ObjectMessage message) {
        ...
        if (jmsObject instanceof Command) {
            ...
            String text = command.getProperty(PROPERTY_NLP_TEXT);
            ...
            LOG.log(Level.INFO, "Nlp recognizes natural text ''{0}'' as command ''{1}''", new Object[]{text, mostSimilar.getName()});
            ...
            try {
                Command resolvedCommand = resolver.resolve(mostSimilar);
                // Schedule the command for execution
                busService.send(resolvedCommand);
                // Reply to the original NLP request
            ...
            } catch (CloneNotSupportedException | VariableResolutionException | JMSException ex) {
                LOG.log(Level.SEVERE, null, ex);
            }
        }
    }
