...
public class UserCommandResource extends AbstractResource<Command> {
    ...
    protected Command prepareSingle(String uuid) {
        // first, search command by UUID
        for (Command oldC : CommandPersistence.getUserCommands()) {
            if (oldC.getUUID().equals(uuid)) {
                return oldC;
            }
        }
        // then search command by name
        if (CommandPersistence.getCommand(uuid) != null) {
            return CommandPersistence.getCommand(uuid);
        }
        return null;
    }
