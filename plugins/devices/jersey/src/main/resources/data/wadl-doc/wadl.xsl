...
<!-- 
 ...
 -->
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0" xmlns:wadl="http://wadl.dev.java.net/2009/02" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:html="http://www.w3.org/1999/xhtml" xmlns="http://www.w3.org/1999/xhtml">
    ...
    <xsl:if test="wadl:grammars/wadl:include">
        ...
        <p>
            <xsl:for-each select="wadl:grammars/wadl:include">
                ...
                <a href="https://raw.githubusercontent.com/ipcsystems/wadl-stylesheet/master/%7B$href%7D"><xsl:value-of select="$href"/></a>
...
<xsl:template name="processResourceSummary">
    ...
    <xsl:if test="wadl:method">
        ...
        <xsl:if test="wadl:method and not($lastResource)">
            <tr><td class="summarySeparator"/><td class="summarySeparator"/><td class="summarySeparator"/></tr>
        </xsl:if>
    </xsl:if>   <!-- wadl:method -->
    ...
    <xsl:for-each select="wadl:doc">
        ...
        <xsl:choose>
            <xsl:when test="@title = 'Example'">
                ...
                <a href="https://raw.githubusercontent.com/ipcsystems/wadl-stylesheet/master/%7B$url%7D"><xsl:value-of select="$url"/></a>
            </xsl:when>
            <xsl:when test="contains($content, 'Example')">
               	<div style=""><pre><xsl:value-of select="."/></pre></div>
            </xsl:when>
            <xsl:otherwise>
...
<xsl:template name="getHyperlinkedElement">
    ...
    <xsl:choose>
        ...
        <xsl:when test="$ns-uri and starts-with($ns-uri, 'http://www.w3.org/XML/') = false">
            <a href="https://raw.githubusercontent.com/ipcsystems/wadl-stylesheet/master/%7B$ns-uri%7D#{$localname}"><xsl:value-of select="$localname"/></a>
        </xsl:when>
        <xsl:when test="$qname">
    ...
    <xsl:if test="wadl:representation">
        ...
        <table>
            <xsl:for-each select="wadl:representation">
                <tr>
                    <td style=""><xsl:value-of select="@mediaType"/></td>
			        <xsl:choose>
	                        ...
	                        <td>
	                            ...
	                            <xsl:choose>
	                                <xsl:when test="@href">
	                                    ...
	                                    <a href="https://raw.githubusercontent.com/ipcsystems/wadl-stylesheet/master/%7B$href%7D"><xsl:value-of select="$localname"/></a>
	                                </xsl:when>
	                                <xsl:otherwise>
						                <xsl:call-template name="getHyperlinkedElement">
