...
public abstract class AbstractTest<Z> extends JerseyTest {
    ...
    @Test
    public void test() {
        Entity<Z> cmdEntity = Entity.entity(c, representation);
        ...
        //GET single
        Z objPre = target(PATH).path(getUuid(c)).request(representation).get(singleType);
        getAssertions(objPre);

        // PUT
        putModifications(objPre);
        Entity<Z> envEntityPut = Entity.entity(objPre, representation);
        Response resPUT = target(PATH).path(getUuid(c)).request().put(envEntityPut);
        assertEquals("PUT test", Status.OK.getStatusCode(), resPUT.getStatus());
        Z objPost = target(PATH).path(getUuid(c)).request(representation).get(singleType);
        putAssertions(objPre, objPost);

        //COPY
        final Response resCOPY = target(PATH).path(getUuid(c)).path("/copy").request(representation).post(null);
        ...
        //DELETE
        Response resDELETE = target(PATH).path(getUuid(c)).request(representation).delete();
        assertEquals("DELETE test", Status.OK.getStatusCode(), resDELETE.getStatus());
        cl = target(PATH).request().get(listType);
        assertEquals("DELETE - Size test", 1, cl.size());
        Response postDELETE = target(PATH).path(getUuid(c)).request().get();
