...
import com.freedomotic.plugins.devices.japi.resources.UserCommandResource;
...
public class UserCommandTest extends AbstractTest<Command>{
    ...
    public void init() throws UriBuilderException, IllegalArgumentException {
        ...
        c.setName("TestCmd");
        c.setUUID(uuid);
        c.setProperty("prop1", "value1");
        c.setReceiver("receiver.channel");
...
assertEquals("Single test - UUID", c.getUUID(), c2.getUUID());
        assertEquals("Single test - NAME", c.getName(), c2.getName());
    }

    @Override
    protected void listAssertions(List<Command> cl) {
       assertEquals("UUID test", c.getUUID(), cl.get(0).getUUID());
        assertEquals("Name test", c.getName(), cl.get(0).getName());
    }
