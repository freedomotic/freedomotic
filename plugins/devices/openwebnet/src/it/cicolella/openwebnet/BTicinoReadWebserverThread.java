...
public class BTicinoReadWebserverThread extends Thread {
    ...
    public void run() {
        ...
        try {
            ...
            } while (true); //in this cycle until result=true or webserver closes the socket;
            //escape when timeout and thread interrupted
        } catch (IOException e) {
            Freedomotic.logger.severe("Exception <ReadThread>: ");
            e.printStackTrace();
        }
        ...
        Freedomotic.logger.info("Thread string receiving stopped"); //FOR DEBUG USE
        socket = null;
        input = null;
        response = null;
    }
